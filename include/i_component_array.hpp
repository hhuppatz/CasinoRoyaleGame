...
{
	assert(entity_to_index_map.find(ent) == entity_to_index_map.end() && "Component added to same entity more than once.");
...
template<typename T>
void component_array<T>::remove_data(entity ent)
{
	assert(entity_to_index_map.find(ent) != entity_to_index_map.end() && "Removing non-existent component.");
...
template<typename T>
T& component_array<T>::get_data(entity ent)
{
	assert(entity_to_index_map.find(ent) != entity_to_index_map.end() && "Retrieving non-existent component.");

	// Return a reference to the entity's component
	return component_array[entity_to_index_map[ent]];
}
